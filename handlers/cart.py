...
async def _show_cart(
    ...
    if isinstance(message_or_callback, types.Message):
        await message_or_callback.answer(text, reply_markup=cart_keyboard(cart_items))
    else:
        await message_or_callback.message.edit_text(
            text, reply_markup=cart_keyboard(cart_items)
        )
...
async def add_to_cart(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Возвращаемся в меню
    await callback.message.edit_text("🍽 Меню:", show_menu_categories())
...
async def clear_cart(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "🍽 Меню:", reply_markup=menu_categories_keyboard(menu)
    )
